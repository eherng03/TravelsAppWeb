...
function getLogInfo(){
	var formData = {
		...
		$.ajax({
			type        : 'GET', 
			url         : '../operations/getLogInfo.php', //archivo que procesa los datos del user
			data        : formData, 
			dataType    : 'json', 
			encode          : true
		}).done(function(data) {
...
function getTripId(){
	var formData = {
		...
		$.ajax({
			type        : 'GET', 
			url         : '../operations/getTripByDriver.php', //archivo que procesa los datos 
			data        : formData, 
			dataType    : 'json', 
			encode          : true
		}).done(function(data) {
			var tripInfo = (data); //obtenemos los datos 
			//TODO Tenemos el id de los viajes del driver, necesitamos los journeys que forman el trip,
			//Y abrimos un chat con los pasajeros de cada trip <3
